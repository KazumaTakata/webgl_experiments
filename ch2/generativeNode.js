...
var FSHADER_SOURCE =
    ...
    gl_FragColor = texture2D(u_Sampler, vec2( v_TexCoord.x + rnd * 0.01  , v_TexCoord.y  + rnd * 0.01 )  ) + 0.2 * vec4( vec3(rnd) , 1)  ;
    }
    `;
...
function draw(gl, n ){
    ...
    gl.uniform2fv(timevar, [ Math.sin(Math.random()) * 100, Math.sin(Math.random()) * 10 ] )
